    ...
    try:
        with st.spinner("Generando historia de usuario..."):
            ...
            if response.status_code == 200:
                ...
                st.session_state.feedback = {"run_id": str(trace.id)}
            else:
...
def create_trace(name, context, requirements, response):
    return client.trace(
        name=name,
...
def generate_experiment_name(include_timestamp=True):
    # Listas de palabras temáticas para IA/ML
    adjectives = [
        "neural",
        "deep",
        "quantum",
